...
import { ChatMessageRepository, GetMessagesByChatId, SendMessage } from "../../domain";
...
export class ChatMessageController {
    ...
    getMessagesByChatId = async (req: Request, res: Response) => {
        try {
            const chat = await new GetMessagesByChatId(this.userRepository).execute(Number(req.params.id));
            if (!chat) {
