...
export class AppRouter {
  static router(): Router {
    ...
    router.use("/api/v1/file", FileUploadRoutes.routes); // NOTE: Add AuthMiddleware.validateJWT after development
    router.use("/api/v1/chats", ChatRouter.router()); // NOTE: Add AuthMiddleware.validateJWT after development
    router.use(
      "/api/v1/donation-transactions",
      DonationTransactionRouter.router()
    ); // NOTE: Add AuthMiddleware.validateJWT after development
    router.use("/api/v1/chat-messages", ChatMessageRouter.router()); // NOTE: Add AuthMiddleware.validateJWT after development
